...
# provide implementation of Javascript client for Elastic Search supporting jQuery and NodeJS clients (basic jQuery support is already implemented).
# provide NodeJS based proxy server for Elastic Search (almost complete now).
...
* It can restrict access to REST API (defaults to @safe@ operations only).
...
* It can round-robin client requests among active http enabled nodes (it periodically refreshes list of active nodes), thus, it can provide a single URL for clients to communicate with Elastic Search cluster. (not implemented yet)